...
tf.flags.DEFINE_integer("embedding_dim", 45, "Dimensionality of character embedding (default: 45)")
tf.flags.DEFINE_string("filter_sizes", "1,2,3", "Comma-separated filter sizes (default: '1,2,3')")
tf.flags.DEFINE_integer("num_filters", 45, "Number of filters per filter size (default: 45)")
...
tf.flags.DEFINE_integer("checkpoint_every", 5, "Save model after this many steps (default: 20)")
...
x_text = [word for word in x_text if word not in stopwords.words('english')]
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        out_dir = os.path.abspath(os.path.join(os.path.curdir, "runs", "built_model"))
        print("Writing to {}\n".format(out_dir))
